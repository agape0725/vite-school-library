...
function BorrowBooks({ books }) {
  return (
    <div className="w-full">
      <ul className="flex flex-wrap gap-5 border-b border-gray-400/40">
        {books.map((book, index) => (
          <BorrowBook key={index} book={book} />
        ))}
      </ul>
    </div>
  );
...
function BorrowBook({ book }) {
  ...
  const { removeBorrowedBook } = useAccounts();
